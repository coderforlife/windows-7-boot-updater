...
#define CHARS_INIT_DATA_SECTION_RW	IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_WRITE
	...
	static const LPVOID GetResourceDirect(const LPVOID data, LPCWSTR type, LPCWSTR name, size_t *size); // must be freed, massively performance enhanced for a single retrieval and no editing // lang? size?
